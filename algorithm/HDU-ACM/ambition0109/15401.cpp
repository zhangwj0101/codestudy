...
////Nickname: Ambition
////Run ID: 
////Submit time: 2010-09-08 15:52:41
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 1540
////Problem Title: 
...
//	if(STree[idx].left == STree[idx].right || STree[idx].acvr == 0 || STree[idx].acvr == STree[idx].Seglen()) {
//		return STree[idx].acvr;
//	}
//	int mid = (STree[idx].left+STree[idx].right)>>1;
//	if(id <= mid) {
//		if(id > STree[(idx)<<1].right - STree[(idx)<<1].rcvr) {
//			return Query(id,(idx)<<1) + Query(STree[(idx)<<1|1].left,(idx)<<1|1);
//		} else {
//			return Query(id,(idx)<<1);
//		}
//	} else {
//		if(id < STree[(idx)<<1|1].left + STree[(idx)<<1|1].lcvr) {
//			return Query(STree[(idx)<<1].right,(idx)<<1) + Query(id,(idx)<<1|1);
//		} else {
...
int Query(int Pos,int idx){
	...
	if(Pos<=mid){
		if(STree[idx<<1].right-STree[idx<<1].rcvr<Pos){
			return Query(Pos,idx<<1) + Query(STree[idx<<1|1].left,idx<<1|1);
		...
		if(STree[idx<<1|1].left+STree[idx<<1|1].lcvr>Pos){
			return Query(STree[idx<<1].right,idx<<1) + Query(Pos,idx<<1|1);
		}else{
