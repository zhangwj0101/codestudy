...
////Nickname: Ambition
////Run ID: 
////Submit time: 2010-09-08 15:55:00
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 1540
////Problem Title: 
...
int Query(int Pos,int idx){
	...
	if(Pos<=mid){
		if(STree[idx<<1].right-STree[idx<<1].rcvr<Pos){
			return Query(Pos,idx<<1) + STree[idx<<1|1].lcvr; //Query(STree[idx<<1|1].left,idx<<1|1);
		}else {
			return Query(Pos,idx<<1);
		}
	}else{
		if(STree[idx<<1|1].left+STree[idx<<1|1].lcvr>Pos){
			return Query(Pos,idx<<1|1)+STree[idx<<1].rcvr;//Query(STree[idx<<1].right,idx<<1);
		}else{
