...
////Nickname: Ambition
////Run ID: 
////Submit time: 2010-11-28 13:00:10
////Compiler: GUN C++
//////////////////////////////////////////////////////
////Problem ID: 3338
////Problem Title: 
	...
	while(~scanf("%d%d",&n,&m)){
			...
			for(int j=0;j<m;j++){
				if(Map[i][j][1]!=-1){
					...
					for(jj=j+1;!Map[i][jj][1]&&jj<m;jj++){
						Insert((i*m+j)*2,(i*m+jj)*2,8);
					}
					Insert(n*m*2,(i*m+j)*2,Map[i][j][1]-(jj-j-1));
					j=jj-1;
				}
			}
		}
			...
			for(int i=0;i<n;i++){
				if(Map[i][j][0]!=-1){
					...
					for(ii=i+1;!Map[ii][j][0]&&ii<n;ii++){
						Insert((ii*m+j)*2,(i*m+j)*2+1,8);
					}
					Insert((i*m+j)*2+1,n*m*2+1,Map[i][j][0]-(ii-i-1));
					i=ii-1;
				}
			}
		}
		...
		for(int i=0;i<n;i++){
			for(int j=0;j<m;j++){
				...
				else printf("%d",9-E[head[(i*m+j)*2]].cap);
			}
			puts("");
		}
	}
