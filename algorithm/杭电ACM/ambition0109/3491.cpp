...
////Nickname: Ambition
////Run ID: 
////Submit time: 2010-12-16 12:41:26
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 3491
////Problem Title: 
...
void Insert(int u,int v,int c,int rc){
	E[node]=Edge(v,c,head[u]);
	head[u]=node++;
	E[node]=Edge(u,rc,head[v]);
	head[v]=node++;
}
...
int main(){
	...
	while(t--){
		...
		for(int i=1;i<=n;i++){
			int cost;
			scanf("%d",&cost);
			Insert(i<<1,i<<1|1,cost,0);
		}
		for(int i=0;i<m;i++){
			int u,v;
			scanf("%d%d",&u,&v);
			Insert(u<<1|1,v<<1,INF,0);
			Insert(v<<1|1,u<<1,INF,0);
		}
