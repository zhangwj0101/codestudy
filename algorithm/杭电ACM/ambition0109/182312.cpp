...
////Nickname: Ambition
////Run ID: 
////Submit time: 2010-09-04 09:09:33
////Compiler: Visual C++
//////////////////////////////////////////////////////
////Problem ID: 1823
////Problem Title: 
...
inline void scan_lf(double &num)
{
...
void cleara(int left,int right,int t,int idx){
	STree[t].act[idx].max=-1;
	if(left==right) return;
	int mid=(left+right)>>1;
	cleara(left,mid,t,idx<<1);
	cleara(mid+1,right,t,idx<<1|1);
}

void clearh(int left,int right,int idx){
	cleara(0,1000,idx,1);
	if(left==right) return;
	int mid=(left+right)>>1;
	clearh(left,mid,idx<<1);
	clearh(mid+1,right,idx<<1|1);
}
